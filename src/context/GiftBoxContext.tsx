...
export const GiftBoxProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [products, setProducts] = useState<Product[]>(() => {
    try {
      const saved = localStorage.getItem('tohfa_products_v18');
  ...
  const [enquiries, setEnquiries] = useState<Enquiry[]>(() => {
    try {
      const saved = localStorage.getItem('tohfa_enquiries_v18');
  ...
  const [giftBoxItems, setGiftBoxItems] = useState<GiftBoxItem[]>(() => {
    try {
      const saved = localStorage.getItem('tohfa_giftbox_v18');
  ...
  useEffect(() => {
    localStorage.setItem('tohfa_products_v18', JSON.stringify(products));
  }, [products]);

  useEffect(() => {
    localStorage.setItem('tohfa_enquiries_v18', JSON.stringify(enquiries));
  }, [enquiries]);

  useEffect(() => {
    localStorage.setItem('tohfa_giftbox_v18', JSON.stringify(giftBoxItems));
  }, [giftBoxItems]);
